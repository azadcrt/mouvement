...
def handle_received_message(data, sender):
    ...
    try:
        ...
        if message_id > last_message_id:
            ...
            path.append(f"{local_eui64.hex()}:{rssi}")#ici
            full_message = create_message(message, message_id_str, path)
            xbee.transmit(xbee.ADDR_BROADCAST, full_message.encode())
            ecrire_log(sender, 1)
            print(f"Message retransmit avec succès: {full_message}")
    except Exception as e:
        print(f"Erreur lors de la gestion du message reçu de: {e}")
        ecrire_log(sender, 0)
