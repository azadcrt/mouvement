...
while idA < 6:
    ...
    if rssi_db < PowerLimit:
        ...
        if (lid < idA):
            ...
            if error == 0 :
                ...
                pource=error/(Nerror+error)
            print("decision en cours")
            bol,Vrssi = algo.decision(lid,pource)
            print("decision prise")
            PowerLimit = Vrssi
            Nerror = 0
            error = 0
            direct = bol
    ...
    if time.ticks_diff(time.ticks_ms(), ltt) >= 500 and nb < 3:
        ltt = time.ticks_ms()
        nb = nb +1
        try:
